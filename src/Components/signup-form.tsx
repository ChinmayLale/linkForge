...
export function SignUpForm({
  ...
  const [signupMethod, setSignupMethod] = useState<"google" | "email" | null>(
    null
  );
    ...
    const checkLoginStatus = async () => {
      ...
      try {
        ...
        if ((session?.needsSignup as boolean) === false && session) {
          toast.success(
            "You are already signed up! Redirecting to your dashboard."
          );
    ...
    try {
      ...
      if (result?.ok) {
        ...
        if (session?.user?.email) {
          ...
          setProgress(1);
          toast.success(
            "Google authentication successful! Please choose a username."
          );
        }
      }
    } catch (error) {
            ...
            <form onSubmit={handleUsernameSubmit}>
              <div className="grid gap-6">
                <div className="grid gap-3">
                  ...
                  <Input
                    id="signup-method"
                    value={`${signupMethod === "google" ? "Google" : "Email"} ${
